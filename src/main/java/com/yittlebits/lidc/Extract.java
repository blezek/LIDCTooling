...
class Extract {
    ...
    static String usage = String.join("\n", "Extract <command> <xml>", "",
	    "Extract information from LIDC XMl and DICOM files", "Commands are:", "  SeriesInstanceUID",
	    "    Extract the SeriesInstanceUID from the XML file (/LidcReadMessage/ResponseHeader/SeriesInstanceUid tog)",
	    "    print it and exit.", "", "  segment <xml> <DICOM Input> <output folder>",
	...
	if (command.toLowerCase().equals("seriesinstanceuid")) {
	    ...
	    String seriesInstanceUID = (String) xpath
		    .compile("/LidcReadMessage/ResponseHeader/SeriesInstanceUid/text()")
