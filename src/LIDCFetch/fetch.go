...
func fetchImage(context *cli.Context) {
	...
	if err != nil {
		logger.Errorf("Error fetching collections")
	}
	defer resp.Body.Close()

	logger.Debugf("Response: %+v", resp)
	if resp.StatusCode != http.StatusOK {
		logger.Errorf("Got bad status '%v' from server", resp.Status)
		os.Exit(1)
	}

	// Do we extract somewhere?
	if context.String("extract") == "" {
		logger.Debugf("Saving to %v", OutputZip)
		f, err := os.Create(OutputZip)
		if err != nil {
			logger.Errorf("Error opening %v for writing: %v", OutputZip, err.Error())
			return
		}
		defer f.Close()
		io.Copy(f, resp.Body)
		...
		logger.Debugf("Extracting to %v", outputDir)
		err := os.MkdirAll(outputDir, os.ModeDir|os.ModePerm)
		if err != nil {
			logger.Errorf("Error creating directory %v: %v", outputDir, err.Error())
			return
		}
		fid, err := ioutil.TempFile(os.TempDir(), "LIDCFetch")
		if err != nil {
			logger.Errorf("Error creating temp file: %v", outputDir, err.Error())
			return
		}
		...
		if err != nil {
			logger.Errorf("Error downloading zip: %v", err.Error())
			return
		}
		fid.Close()
		logger.Debugf("Finished saving to %v", fid.Name())
		// Unzip in the specified location
		z, err := zip.OpenReader(fid.Name())
		if err != nil {
			logger.Errorf("Error opening zip archive: %v", err.Error())
			return
		}
		defer z.Close()
		// Loop over the files
		for _, f := range z.File {
			...
			logger.Infof("Extracting %v Saving %v", f.Name, extractedFilename)
			if strings.HasSuffix(f.Name, "/") {
				// Create the directory
				logger.Debugf("Making directory %v", extractedFilename)
				err = os.MkdirAll(extractedFilename, os.ModeDir|os.ModePerm)
				if err != nil {
					logger.Errorf("Error creating directory %v: %v", extractedFilename, err.Error())
					return
				}
				continue
			}
			...
			if err != nil {
				logger.Errorf("Error opening output file %v: %v", extractedFilename, err.Error())
				return
			}
			defer efid.Close()
			zfid, err := f.Open()
			if err != nil {
				logger.Errorf("Error opening zip file %v: %v", f.Name, err.Error())
				return
			}
			defer zfid.Close()
			_, err = io.Copy(efid, zfid)
			if err != nil {
				logger.Errorf("Error extracting zip file %v to %v: %v", f.Name, extractedFilename, err.Error())
				return
			}
		}
